...
//        pose.ifPresent(estimatedRobotPose -> SmartDashboard.putString("Closest point: ", getClosestScoringPoint(estimatedRobotPose).toString()));
  ...
  public double launcherAngleFromRange(Pose2d pose) {
      int low = (int) distanceFromGoal(pose);
      int high = (int) distanceFromGoal(pose) + 1;
      double weight = distanceFromGoal(pose) % 1;
      return Constants.anglesFromRange.get(low) * (1 - weight) + Constants.anglesFromRange.get(high) * weight;
  }

  public double distanceFromGoalNew(Pose2d pose) {
    switch (DriverStation.getAlliance().orElse(DriverStation.Alliance.Blue)) {
      case Blue:
        LimelightHelpers.setPriorityTagID(Constants.Ports.CAMERA, 7);
        if (LimelightHelpers.getFiducialID(Constants.Ports.CAMERA) == 7) {
          distanceFromGoal(pose);
        } else {
          return -1;
        }
      case Red:
        LimelightHelpers.setPriorityTagID(Constants.Ports.CAMERA, 4);
        if (LimelightHelpers.getFiducialID(Constants.Ports.CAMERA) == 4) {
          distanceFromGoal(pose);
        } else {
          return -1;
        }
  ...
  public Transform2d lineUpWithGoal(Pose2d pose) {
      return switch (DriverStation.getAlliance().orElse(DriverStation.Alliance.Blue)) {
          case Blue -> {
              LimelightHelpers.setPriorityTagID(Constants.Ports.CAMERA, 7);
              yield new Transform2d(0, 0, Rotation2d.fromRadians(LimelightHelpers.getTX(Constants.Ports.CAMERA)));
          }
          case Red -> {
              LimelightHelpers.setPriorityTagID(Constants.Ports.CAMERA, 4);
              yield new Transform2d(0, 0, Rotation2d.fromRadians(LimelightHelpers.getTX(Constants.Ports.CAMERA)));
