...
//        pose.ifPresent(estimatedRobotPose -> SmartDashboard.putString("Closest point: ", getClosestScoringPoint(estimatedRobotPose).toString()));
  ...
  /**
    * Gets a coordinate point representing the closest scoring goal, in inches
    * FIXME: change from inches to meters
    * @return A Pose2d representing the closest scoring goal
   **/
  public Pose2d getClosestScoringPoint(Pose2d pose) {
    double xPoint = 0;
    double yPoint = 0;
    Rotation2d currentRotation = pose.getRotation();
    if (distanceFromGoal(pose) <= Constants.CAMERA_RANGE) {
      return switch (DriverStation.getAlliance().orElse(DriverStation.Alliance.Blue)) {
        case Blue:
          xPoint = Constants.SCORING_RANGE * Math.cos(angleRelativeToGoal(pose)) - Units.inchesToMeters(1.5);
          yPoint = Constants.SCORING_RANGE * Math.sin(angleRelativeToGoal(pose)) + Units.inchesToMeters(218.42);
          yield new Pose2d(xPoint, yPoint, currentRotation);
        case Red:
          xPoint = Constants.SCORING_RANGE * Math.cos(angleRelativeToGoal(pose)) + Units.inchesToMeters(652.73);
          yPoint = Constants.SCORING_RANGE * Math.sin(angleRelativeToGoal(pose)) + Units.inchesToMeters(218.42);
          yield new Pose2d(xPoint, yPoint, currentRotation);
      };
    }
  ...
  public Transform2d getToScoringPosition(Pose2d pose) {
    ...
    if (distanceFromGoal(pose) <= Constants.SCORING_RANGE) {
    ...
    } else if (distanceFromGoal(pose) <= Constants.CAMERA_RANGE) {
